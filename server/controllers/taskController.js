...
const deleteRestoreTask = asyncHandler(async (req, res) => {
    try {
      ...
      if (actionType === "delete") {
        if (!id) {
          return res.status(400).json({ status: false, message: "Task ID required for delete." });
        }
        ...
        if (!id) {
          return res.status(400).json({ status: false, message: "Task ID required for restore." });
        }
        ...
        return res.status(400).json({ status: false, message: "Invalid actionType." });
      }
